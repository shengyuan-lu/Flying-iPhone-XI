...
public class SuccessScene:SKScene {
    ...
    // Foreground
    let foreground = SKSpriteNode(imageNamed: "success")
    
    // Sound
    let click = SKAction.playSoundFileNamed("Click.wav", waitForCompletion: false)
    let win = SKAction.playSoundFileNamed("win.wav", waitForCompletion: false)
    ...
    public override func didMove(to view: SKView) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
            ...
            self.continueButtonLabel.text = "About"
        ...
        for touch in touches {
            ...
            if continueButton.contains(location) {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                    // Create scene three
                    let sceneFive = aboutScene(fileNamed: "aboutScene")
                    sceneFive?.scaleMode = .aspectFill
                    sceneFive?.anchorPoint = CGPoint.zero
                    
                    // Change scene
                    self.view?.presentScene(sceneFive!, transition: SKTransition.crossFade(withDuration: 1.0))
                }
            }
        }
    }
